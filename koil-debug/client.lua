...
function GetObject()
    ...
    repeat
        ...
        if canPedBeUsed(ped) and distance < 30.0 and (distanceFrom == nil or distance < distanceFrom) then
	    	...
	    	zdicks = -1184516519
	    	zdicks2 = GetEntityModel(ped)
	    	if zdicks == zdicks2 then
	    		--
	    	end
            if lowGrav then
            	--ActivatePhysics(ped)
...
function getNPC()
    ...
    repeat
        ...
        if canPedBeUsed(ped) and distance < 30.0 and (distanceFrom == nil or distance < distanceFrom) then
	    	...
	    	if IsEntityTouchingEntity(GetPlayerPed(-1), ped) then
	    		DrawText3Ds(pos["x"],pos["y"],pos["z"], "Ped: " .. ped .. " Model: " .. GetEntityModel(ped) .. " IN CONTACT" )
	    	else
	    		DrawText3Ds(pos["x"],pos["y"],pos["z"], "Ped: " .. ped .. " Model: " .. GetEntityModel(ped) .. "" )
	    	end
